...
public class ReplyController {
    ...
    @PostMapping("/{postId}")
    public ResponseEntity<ResponseDto<Long>> saveReply(@AuthenticationPrincipal Member member, @Valid @RequestBody ReplyDto replyDto, @Parameter(name = "postId",description = "댓글을 작성할 게시글의 id",in = ParameterIn.PATH)@PathVariable Long postId) throws NoSuchAlgorithmException {
        log.info("댓글 등록 호출");
        return ResponseEntity.status(HttpStatus.CREATED).body(ResponseDto.of(replyCommandService.saveReply(member, replyDto, postId),"댓글 등록 성공"));
    }
    ...
    public ResponseEntity<ResponseDto<Long>> deleteReply(@AuthenticationPrincipal Member member, @Parameter(name = "replyId",description = "삭제할 댓글의 id",in = ParameterIn.PATH)@PathVariable Long replyId){
        log.info("댓글 삭제 호출 id:{}",replyId);
        replyCommandService.delete(member.getId(), replyId);
    ...
    public ResponseEntity<ResponseDto<Long>> saveReReply(@AuthenticationPrincipal Member member, @Valid @RequestBody ReplyDto replyDto, @Parameter(name = "replyId",description = "대댓글을 작성할 댓글의 id",in = ParameterIn.PATH)@PathVariable Long replyId) throws NoSuchAlgorithmException {
        log.info("댓글 저장 호출");
